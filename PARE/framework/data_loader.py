    ...
    def eval(self, pred_result, threshold=0.5):
        ...
        for i, item in enumerate(sorted_pred_result):
            ...
            if idtf not in entpair:
                entpair[idtf] = {
                    'label': np.zeros((len(self.rel2id)), dtype=int),
                    'pred': np.zeros((len(self.rel2id)), dtype=int),
                    'score': np.zeros((len(self.rel2id)), dtype=float),
                    'prediction':[]
                }
            if (item['entpair'][0], item['entpair'][1], item['relation']) in self.facts:
                correct += 1
        ...
        recall_at_5_micro = 0.0
        recall_at_5_macro = 0.0
        if total > 0:
            hits = 0
            macro_vals = []
            for ep in entpair:
                label = entpair[ep]['label']
                score = entpair[ep]['score']
                gold_rel_ids = np.where(label[1:] == 1)[0] + 1
                if gold_rel_ids.shape[0] == 0:
                    continue
                topk = min(5, len(score) - 1)
                top_rel_ids = (np.argsort(score[1:])[::-1][:topk]) + 1
                ep_hits = int(np.isin(gold_rel_ids, top_rel_ids).sum())
                hits += ep_hits
                macro_vals.append(float(ep_hits) / float(gold_rel_ids.shape[0]))
            recall_at_5_micro = float(hits) / float(total)
            if len(macro_vals) > 0:
                recall_at_5_macro = float(np.mean(macro_vals))
