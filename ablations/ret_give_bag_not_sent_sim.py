...
# ============================================================================
# BERTSCORE COMPUTATION - ABLOTION 1: NO SEMANTIC SIMILARITY
# ============================================================================
# In this ablation, the entire embedding computation loop is skipped.
# sent_scores stays all zeros. Only PARE scores are used for retrieval.

def compute_bertscore_for_query(queries, doc_bags):
    """
    Compute scores for all queries and document sets.
    ABLOTION 1: Skips BERTScore/sentence embedding computation entirely.
    sent_scores stays all zeros. Only PARE scores are used for retrieval.
    Args:
    ...
    print("Computing scores for all queries and document sets (NO SEMANTIC SIMILARITY - Ablation 1)...")
    ...
    # Compute PARE scores (this is the ONLY scoring used in this ablation)
    ...
    # ABLOTION: Skip all BERTScore/sentence embedding computation.
    # sent_scores stays all zeros, only PARE scores drive retrieval.
    # (Lines 270-285 of main pipeline are commented out here.)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="HYDRE Bag Selection (Stage 2) - Ablation 1: Without Semantic Similarity")
    ...
    print("=" * 60)
    print("HYDRE RETRIEVAL SYSTEM - ENGLISH (ABLOTION 1: NO SEMANTIC SIMILARITY)")
    print("=" * 60)
    ...
    output_file = os.path.join(OUT_DIR, f"not_sent_sim_{TOPK}_bag_pare5_{os.path.basename(RETRIEVAL_MODEL_NAME)}_nonna_en_distinct_msk{MASK}_MAX_sent_nottune.json")
