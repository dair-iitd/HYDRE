...
# ============================================================================
# SINGLE SENTENCE SELECTION FROM BAGS - ABLOTION 4: RANDOM SCORES
# ============================================================================
# In this ablation, random scores are used instead of real PARE scores
# for sentence selection. The selection logic stays the same (threshold
# crossing + direct sum), but scores are random.

def best_for_each_rel(all_bag_data, rel2id):
    """
    Select the best sentence from each bag based on random scores (ablation).
    ABLOTION 4: Generates random scores instead of computing real PARE scores.
    Args:
        all_bag_data: List of bag data containing multiple sentences per bag
        rel2id: Dictionary mapping relation names to IDs
    """
    print("Ablation 4: Using random scores for sentence selection...")

    # Get number of relations from rel2id
    num_relations = len(rel2id)

    sent_idx = 0
    for comb_data in tqdm(all_bag_data, desc="Processing bags (random scores)"):
        all_example_bags = comb_data["top_docs"]
        for bag_data in all_example_bags:
            next_sent_idx = sent_idx + len(bag_data["texts"])

            # ABLOTION 4: Generate random scores instead of real PARE scores
            # (Lines 182-183 of source abl files)
            pare_scores_batch = torch.rand((next_sent_idx - sent_idx, num_relations))

            useful_relations = [rel2id[x] for x in bag_data["relations"]]
            threshold = 0.5

            # Calculate scores for each sentence (same logic as main pipeline)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="HYDRE Single Example Selection (Stage 3) - Ablation 4: Without Stage 3 (Random Scores)")
    ...
    print("=" * 60)
    print("HYDRE SINGLE SENTENCE SELECTION - ENGLISH (ABLOTION 4: RANDOM SCORES)")
    print("=" * 60)
    ...
    # Output filename: replace _single_selected.json with _random_single_selected.json
    out_file = os.path.join(OUT_DIR, os.path.basename(LARGER_BAG_FILE).replace(".json", "_random_single_selected.json"))
    ...
    # ========================================================================
    # PROCESS BAGS - SELECT BEST SENTENCES (RANDOM SCORES)
    # ========================================================================

    print("Processing bags to select best sentences (random scores)...")
